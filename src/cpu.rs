...
impl CPU {
    ...
    pub fn cpu_cycle(&mut self) -> u8 {
        ...
        match op {
            ...
            0xc2 => { self.jump(self.fetch_ins_word(), Some(Condition::NZ)) as u8 + 3 },
            ...
            0xca => { self.jump(self.fetch_ins_word(), Some(Condition::Z)) as u8 + 3 },
            ...
            0xd2 => { self.jump(self.fetch_ins_word(), Some(Condition::NC)) as u8 + 3 },
            ...
            0xda => { self.jump(self.fetch_ins_word(), Some(Condition::C)) as u8 + 3 },
            ...
            0xf8 => { let old_sp = self.regs.sp(); self.add_word(self.fetch_ins_byte() as i8 as i16 as u16, RegIndex::SP); self.ld_word(self.regs.sp(), Some(RegIndex::HL), 0); self.regs.set_sp(old_sp); self.regs.set_zflag(false); 3 },
            0xf9 => { self.ld_word(self.regs.hl(), Some(RegIndex::SP), 0); 2 },
            0xfa => { self.ld_byte(self.mmu.read_byte(self.fetch_ins_word()), Some(RegIndex::A), 0); 4 },
            0xfb => { self.ei(); 1 },
